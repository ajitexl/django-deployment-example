...
def deco(info):
    def ensure_str(s, encoding='latin-1', errors='ignore'):
        """Coerce *s* to `str`.
        For Python 2:
...
def hammer(request):
	...
	identity = "Hammer"
	hammer_data = {}
	hammer_data['output'] = output
	#print(hammer_data['output'])
	hammer_data['page'] = identity
	hammer_data['path'] = ['Home']
	if identity not in hammer_data['path']:
		hammer_data['path'].append(identity)
	return render(request, 'hammer/hammer.html', hammer_data)
